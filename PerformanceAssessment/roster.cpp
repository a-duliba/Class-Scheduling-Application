...
//Add new student to array
	...
	int index = stoi(studentID.substr(1)) - 1; //Convert string to int and fill in the accessors calling the appropriate methods
	...
	for (int i = 0; i < 5; i++)
	{
		...
		if (classRosterArray[i])
		{
			...
			if (classRosterArray[i]->getStudentID() == studentID)
			{
				...
				//Set the avgDaysInCourse to adding the arrayand dividing by 3
					cout << "Average time take for: " << studentID << " to finish 3 courses: " << avgDaysInCourse << " days." << endl; 
				//Print the average time to finish 3 courses
			}
		}
	}
}
...
void Roster::printInvalidEmails()
{
	...
	bool space = false;
	bool period = false;
	cout << "Invalid Emails Listed are: " << endl;
	//Print title
...
void Roster::printAll()
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		if (classRosterArray[i])
		{
			...
			{
				classRosterArray[i]->print();
			//Call the print function to print the student to console
			}
		}
	}
}
